...
const UserMapElement = ({ userUId }: { userUId: string }) => {
	const { data: CurrentUser, isLoading, error } = useFetchSingleUserQuery(userUId)
	...
	return <View>
		<Pressable onPress={() => navigate('AutorProfile', {
			...
			<Text numberOfLines={1}
			      className='mt-1 max-w-[80px] text-center text-white font-bold'>{CurrentUser.name}</Text>
		</Pressable>
	</View>
}
