...
const CommentElement: FC<Comment> = (props) => {
	...
	return <View>
		<Pressable onPress={() => navigate('AutorProfile', {
			...
			{user.photoURL ?
				<Image source={{ uri: user.photoURL }} className='w-[70px] mr-3 h-[70px] rounded-full' /> :
				<ClearUserLogo letter={user.name} width={70} height={70} />}
			
			<View className='flex-1 ml-2'>
				<View className='flex-row justify-between items-center'>
					<Text numberOfLines={1}
					      className='text-white font-bold text-xl flex-wrap max-w-[190px] whitespace-normal'>{user.name}</Text>
